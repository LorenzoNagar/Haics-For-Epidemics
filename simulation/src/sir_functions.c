...
void SIR_CVODES_Incidence(double results_y[][g_inpset->num_out_cvodes], double results_s[][g_inpset->num_out_cvodes][kNumParam - 2], double state_params[kNumParam - 2],
  ...
  if(iS == 0){
    ...
    for (iii = 3; iii < kNumParam - 2; iii++){
      if(iS == iii){
        ...
        sd[NEQ - 1] += beta_t * data->basis[iii - 2] * y1 * y_tot / num_pop;
      }
    }
  }
