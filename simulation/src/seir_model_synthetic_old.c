...
double PriorSEMIKR(){
    ...
    t_s0 =     1.0 + (num_pop - 1)/(1 + exp(-state[curr]->pos[2]));
    t_e0 =     1.0 + (num_pop - 1)/(1 + exp(-state[curr]->pos[3]));
    t_i0 =     1.0 + (num_pop - 1)/(1 + exp(-state[curr]->pos[4]));
...
double LogJacobianSEMIKR(){
    ...
    double t_i0 = 1.0 + (num_pop - 1) / (1 + exp(-state[curr]->pos[2]));
    // Derivative of t_i0 with respect to st->pos[1]
    double exp_neg_pos1 = exp(-state[curr]->pos[2]);
    double denom1 = (1 + exp_neg_pos1) * (1 + exp_neg_pos1);
    double J2 = (num_pop - 1) * exp_neg_pos1 / denom1;
    // Check for NaN or Inf
...
void TransfSEMIKR() {
    ...
    // Transform initial infected count parameter t_i0 (st->pos[1])
    // Original transformation: t_i0 = 1.0 + (num_pop - 1.0) / (1 + exp(-st->pos[2]))
    // Inverse transformation: st->pos[2] = log((t_i0 - 1.0) / (num_pop - t_i0))
    double t_i0 = state[curr]->pos[2];  // Current value of t_i0
    state[curr]->pos[2] = log((t_i0 - 1.0) / (num_pop - t_i0));
...
void InvTransfSEMIKR() {
    ...
    // Transformation: t_i0 = 1.0 + (num_pop - 1.0) / (1.0 + exp(-st->pos[2]))
...
void GradientDescentSEMIKR(){
    int i, j, ii;
    int init_attempts=0, init_max_attempts=4;
    ...
    while(!isfinite(logPosterior)){
        if(init_attempts <= init_max_attempts){
            ...
            for (i = 0; i < g_inpset->num_basis_spline; i++) {
                ...
                prior_settings[5 + i].param1 = 1;       // a
                prior_settings[5 + i].param2 = 2;    // b
            }
...
int ReadModelSEMIKR_Incidence() {
    ...
    if((bool_initialpointfile_exists == 0) || (bool_splinebasis_exists == 0)){
        ...
        int total_params = 5 + g_inpset->num_basis_spline;
        ...
        // Initialize spline coefficients (state[curr]->pos[5] onwards)
        for (i = 0; i < g_inpset->num_basis_spline; i++) {
            // Assuming the prior for spline coefficients is defined in prior_settings[4 + i]
            state[curr]->pos[5 + i] = -beta_0;
            // state[curr]->pos[5 + i] = sample_from_prior(&prior_settings[5 + i]);
        }
