...
double PriorSIKR_Incidence(){
    ...
    double m[] = {g_inpset->gamma_param_1, g_inpset->I0_param_1, 0.1};
    double s[] = {g_inpset->gamma_param_2, g_inpset->I0_param_2, 75e-4};

    double logp = 0.0;
    double params[3] = {t_gamma, t_i0, t_phi_inv};
...
double LoglikelihoodSIKR_Incidence(){
    ...
    double results_s[NNEQ][kNumObs][2 + g_inpset->num_basis_spline];
...
void GradientSIKR_Incidence(TypeState *st){
    ...
    SIR_CVODES_Incidence(results_y, results_s, parameters_odes, g_inpset->spline_pol_degree, mm, nf, knots, basis, sensitivities);
...
void TransfSIKR_Incidence() {
    ...
    state[curr]->pos[3] = log(state[curr]->pos[3]);
}
...
void InvTransfSIKR_Incidence() {
    ...
    state[curr]->pos[3] = exp(state[curr]->pos[3]);
}
...
void GradientDescentSIKR_Incidence(){
...
int ReadModelSIKR_Incidence() {
    ...
    kNumParam = 4 + g_inpset->num_basis_spline;
    ...
    if((bool_initialpointfile_exists == 0) || (bool_splinebasis_exists == 0)){
        ...
        if(g_inpset->num_basis_spline != numSizeSplineBasis){
            fprintf(stderr, "sir_model_synthetic.c - num_basis_spline (%d) is not equal to numSizeSplineBasis (%i) in sir_model_synthetic.c. Exiting...\n", g_inpset->num_basis_spline, numSizeSplineBasis);
            exit(EXIT_FAILURE);
        }

        if(kNumParam != numSizeInitialPoint + numSizeSplineBasis){
            fprintf(stderr, "sir_model_synthetic.c - kNumParam (%d) is not equal to numSizeInitialPoint + numSizeSplineBasis (%i) in sir_model_synthetic.c. Exiting...\n", kNumParam, numSizeInitialPoint + numSizeSplineBasis);
            exit(EXIT_FAILURE);
        }
